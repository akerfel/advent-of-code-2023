...
public class Part2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<Long> sourcesList = new ArrayList<>();
        for (int i = 0; i < sourcesStr.length - 1; i += 2) {
            long start = Long.parseLong(sourcesStr[i]);
            long range = Long.parseLong(sourcesStr[i + 1]);
            for (int j = 0; j < range; j++) {
                sourcesList.add(start + j);
            }
        }
        long[] sources = sourcesList.stream().mapToLong(x -> x).toArray();
